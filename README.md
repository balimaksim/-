...
Стоит отметить, что с 30 ноября 2024 года в России действует закон, запрещающий информацию о VPN-сервисах, используемых для обхода блокировок, за исключением VPN для безопасного удаленного доступа.
Мы предупреждаем, что в этом проекте мы рассматриваем технологию VPN, только как средство создания собственной безопасности в сети интернет.
...
Установка производилась на сервер с ОС Ubuntu 20.04. Для других версий процесс может отличаться. Подключитесь к серверу по SSH для начала установки.
...
2. Установите необходимые пакеты для OpenVPN:
...
1. Скопируйте файлы EasyRSA в папку OpenVPN и перейдите в неё:

		sudo cp -R /usr/share/easy-rsa /etc/openvpn/
		cd /etc/openvpn/easy-rsa 

2. Инициализируйте удостоверяющий центр и создайте корневой сертификат:
...
1. Создайте директорию для хранения сертификатов и переместите туда корневой сертификат:

		mkdir /etc/openvpn/certs/
		cp /etc/openvpn/easy-rsa/pki/ca.crt /etc/openvpn/certs/ca.crt

## Генерация сертификатов и ключей для сервера

1.Создайте закрытый ключ и запрос сертификата для сервера:

		./easyrsa gen-req server nopass
2. Подпишите сертификат сервера:
...
3. Скопируйте сертификат и ключ сервера в папку certs:
...
4. Сгенерируйте параметры Диффи-Хеллмана:
		
		openssl dhparam -out /etc/openvpn/certs/dh2048.pem 2048

5. Создайте ключ HMAC:
...
1. Создайте закрытый ключ и запрос сертификата для клиента:

		./easyrsa gen-req client1 nopass
2. Подпишите сертификат клиента:
...
## Заключение
На этом моменте наша статья подходит к концу. Не забывайте про свою безопасность в Интернете и пользуйтесь VPN. Досвидиания!!!
